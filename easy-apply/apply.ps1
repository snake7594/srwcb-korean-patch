#requires -version 3
# 슈퍼로봇대전 컴플리트 박스 한글패치 v0.10.8 (제2차 + 제3차 + EX 전체) 적용 엔진
...
$patch   = Join-Path $root 'srwcb-second-third-ex-korean-v0.10.8.xdelta'

$T1NAME  = 'Super Robot Taisen Complete Box (Track 1).bin'
$T2NAME  = 'Super Robot Taisen Complete Box (Track 2).bin'
$OUTNAME = 'Super Robot Taisen Complete Box Korean v0.10.8 (Track 1).bin'
$CUENAME = 'Super Robot Taisen Complete Box Korean v0.10.8.cue'

$EXP_SRC   = '3f25650b588774d55c3bbb5b771779beab408eaca020e9a622133ade323a0f94'
$EXP_OUT   = 'ea7dea63a8956d7bb2f15d7545ee7f13441443c6a198885aedfce63a43c2a01f'
$EXP_PATCH = '420cd3b41937e7b3f5d941200118ac6e5f392ffebc015e7f6acc5830a8f6d548'
...
try {
    Write-Host ''
    Write-Host '============================================================'
    Write-Host '   슈퍼로봇대전 컴플리트 박스 한글패치 v0.10.8'
    Write-Host '   (제2차 + 제3차 + EX 전체)'
    Write-Host '============================================================'
    Write-Host ''

    if (-not (Test-Path -LiteralPath $xdelta)) { Fail "xdelta.exe 가 없습니다. 패치 파일들을 한 폴더에 함께 두세요." }
    if (-not (Test-Path -LiteralPath $patch))  { Fail "srwcb-second-third-ex-korean-v0.10.8.xdelta 가 없습니다." }
