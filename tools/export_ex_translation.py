...
OUT_DIR = _P.LEDGER
LEDGER = OUT_DIR / "ex_translation_ledger.json"
GLOSSARY = OUT_DIR / "ex_glossary_candidates.json"
EXTRACTED = _P.EXTRACTED

TARGETS = {
    "EX/E_SCE.BIN": "scenario",
    "BMESS4.BIN": "battle_message",
    "EX/E_DEAD.BIN": "death_quote",
}
...
def select_authoritative_candidates(
    ...
    for candidate in ordered_by_source["EX/E_SCE.BIN"]:
    ...
    bmess_data = (EXTRACTED / "BMESS4.BIN").read_bytes()
    ...
    for block in bmess.blocks:
        ...
        for target, record in sorted(block.text_records.items()):
            ...
            candidate, synthesized = runtime_candidate(
                by_source["BMESS4.BIN"].get(offset),
                source_path="BMESS4.BIN",
                offset=offset,
                raw=raw,
    ...
    for candidate in ordered_by_source["BMESS4.BIN"]:
        offset = candidate["offset"]
    ...
    dead_data = (EXTRACTED / "EX" / "E_DEAD.BIN").read_bytes()
    ...
    for start, record in sorted(dead.records.items()):
        ...
        candidate, synthesized = runtime_candidate(
            by_source["EX/E_DEAD.BIN"].get(start),
            source_path="EX/E_DEAD.BIN",
            offset=start,
            raw=raw,
    ...
    for candidate in ordered_by_source["EX/E_DEAD.BIN"]:
