...
리소스 형식 (게임 자체 포맷. TIM 이 아니다)
    ...
    ★ 레코드의 데이터 오프셋은 **압축 해제 결과 시작보다 2바이트 앞**을 기준으로
      한다(게임이 dest 앞 2바이트를 따로 쓰는 것으로 보인다). 그래서 우리 버퍼
      에서는 `off - 2` 가 참 위치다. 근거는 두 구조 불변식이 -2 에서만 맞는다는
      것이다(m171 만 꼬리 1바이트 여유):
          마지막레코드.off + w*h/2 == len(raw)
          레코드표 끝 ~ 첫 픽셀 간격 == 4 + 32 * CLUT수
      보정 없이 재면 각각 정확히 2 / 32의 배수 아님 으로 어긋난다.
      이걸 몰라서 v0.11.33 까지 모든 스프라이트를 4픽셀(=2바이트) 늦게 읽고 썼다.
      그 결과 (a) 우리가 그린 글자가 오른쪽으로 4px 밀려 각 행 끝 4px 이 다음 행
      맨 앞으로 감기고(제보 #15b·#18c), (b) 쓰기의 마지막 2바이트가 늘 **다음
      레코드**(영문 부제·작은 숫자) 첫 4픽셀을 덮어썼다(52개 레코드 58바이트).
...
#: 레코드 오프셋 기준점과 압축 해제 버퍼 시작의 차이(모듈 독스트링 참조).
TAG_BYTES = 2
...
def parse(raw):
    ...
    for i in range(n):
        ...
        out.append({"i": i, "off": off - TAG_BYTES, "file_off": off,
                    "a": a, "b": b, "w": w, "h": h,
                    "px": px, "py": py})
    return out, 2 + 16 * n
