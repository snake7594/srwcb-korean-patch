...
def _mid_pad(body, pad):
    ...
    i = body.rfind(b"\xF6")
    if i < 0: return bytes(body) + b"\x00" * pad
    return bytes(body[:i + 1]) + b"\x00" * pad + bytes(body[i + 1:])
_fn = 0
for _tt, _kb in _FOREIGN.items():
    _et = _tt - _TH2EX
    _te = rec_end(RETAIL, _et)
    assert RETAIL[_et:_te] == _THRET[_tt:_tt + (_te - _et)], \
        f"외래 레코드 {hex(_et)} 레트일 바이트가 제3차와 다릅니다"
    _orig = _te - _et
    assert war[_te - 1] == 0xFF
    _body = _kb[:-1] if _kb[-1] == 0xFF else _kb
    assert len(_body) <= _orig - 1, f"외래 레코드 {hex(_et)} 넘침 {len(_body)}>{_orig-1}"
    war[_et:_et + _orig - 1] = _mid_pad(_body, _orig - 1 - len(_body))
    _fn += 1
print(f"  외래 맵/시스템 레코드 {_fn}개 제자리 번역 (중간패딩)")
