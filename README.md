...
`v0.2.1-pre`는 《슈퍼로봇대전 컴플리트 박스》에 수록된
《제2차 슈퍼로봇대전》의 대사와 메뉴·인터미션을 한국어로 바꾸는
메뉴 런타임 문제를 수정한 조기 시험판입니다.
...
역적용 검증을 통과했습니다. 메뉴 문자열이 실행 중 작업 버퍼에 덮어써지던
문제를 고쳤습니다. 다만 처음부터 끝까지의 실기 플레이 검증은
...
.\xdelta.exe -d -s `
  ...
  ".\release\srwcb-second-korean-v0.2.1-pre.xdelta" `
...
- 원본 실행 파일의 루트 블록은 실행 파일 내부 cave로 옮기고, 메뉴 영역이
  전투용 scratch와 겹치지 않도록 보호
...
릴리스별 상세 내용은
[`v0.2.1-pre` 릴리스 노트](docs/RELEASE_NOTES_v0.2.1-pre.md)에
기록되어 있습니다. 이전 시험판의 기록은
[`v0.2.0-pre` 릴리스 노트](docs/RELEASE_NOTES_v0.2.0-pre.md)에서 확인할 수
있습니다.
