...
OUT_DIR = _P.LEDGER
LEDGER = OUT_DIR / "third_translation_ledger.json"
GLOSSARY = OUT_DIR / "third_glossary_candidates.json"
EXTRACTED = _P.EXTRACTED

TARGETS = {
    "THIRD/3_SCE.BIN": "scenario",
    "BMESS3.BIN": "battle_message",
    "THIRD/3_DEAD.BIN": "death_quote",
}
...
def select_authoritative_candidates(
    ...
    for candidate in ordered_by_source["THIRD/3_SCE.BIN"]:
    ...
    bmess_data = (EXTRACTED / "BMESS3.BIN").read_bytes()
    ...
    for block in bmess.blocks:
        ...
        for target, record in sorted(block.text_records.items()):
            ...
            candidate, synthesized = runtime_candidate(
                by_source["BMESS3.BIN"].get(offset),
                source_path="BMESS3.BIN",
                offset=offset,
                raw=raw,
    ...
    for candidate in ordered_by_source["BMESS3.BIN"]:
        offset = candidate["offset"]
    ...
    dead_data = (EXTRACTED / "THIRD" / "3_DEAD.BIN").read_bytes()
    ...
    for start, record in sorted(dead.records.items()):
        ...
        candidate, synthesized = runtime_candidate(
            by_source["THIRD/3_DEAD.BIN"].get(start),
            source_path="THIRD/3_DEAD.BIN",
            offset=start,
            raw=raw,
    ...
    for candidate in ordered_by_source["THIRD/3_DEAD.BIN"]:
