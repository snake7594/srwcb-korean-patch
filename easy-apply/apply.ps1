#requires -version 3
# 슈퍼로봇대전 컴플리트 박스 한글패치 v0.10.7 (제2차 + 제3차 + EX 전체) 적용 엔진
...
$patch   = Join-Path $root 'srwcb-second-third-ex-korean-v0.10.7.xdelta'

$T1NAME  = 'Super Robot Taisen Complete Box (Track 1).bin'
$T2NAME  = 'Super Robot Taisen Complete Box (Track 2).bin'
$OUTNAME = 'Super Robot Taisen Complete Box Korean v0.10.7 (Track 1).bin'
$CUENAME = 'Super Robot Taisen Complete Box Korean v0.10.7.cue'

$EXP_SRC   = '3f25650b588774d55c3bbb5b771779beab408eaca020e9a622133ade323a0f94'
$EXP_OUT   = 'ce02b13e9b546c5634982420257107bbb31907e5fc3be4e6e013e5447fcf430c'
$EXP_PATCH = '695d3b6dae96afc55ae9d6de983720250251ee1a0b7dd30869dad6f24bae1886'
...
try {
    Write-Host ''
    Write-Host '============================================================'
    Write-Host '   슈퍼로봇대전 컴플리트 박스 한글패치 v0.10.7'
    Write-Host '   (제2차 + 제3차 + EX 전체)'
    Write-Host '============================================================'
    Write-Host ''

    if (-not (Test-Path -LiteralPath $xdelta)) { Fail "xdelta.exe 가 없습니다. 패치 파일들을 한 폴더에 함께 두세요." }
    if (-not (Test-Path -LiteralPath $patch))  { Fail "srwcb-second-third-ex-korean-v0.10.7.xdelta 가 없습니다." }
